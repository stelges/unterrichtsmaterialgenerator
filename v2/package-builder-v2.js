...
function buildDidacticPlan(data) {
  const paths = {
    [TEMPLATE_MODES.KNOWLEDGE]: ["Stundenfrage verstehen", "Wissen aufbauen", "Beispiel nachvollziehen", "anwenden", "sichern"],
    ...
    [TEMPLATE_MODES.WORKSHEET]: ["Frage verstehen", "Beispiel nutzen", "Aufgaben bearbeiten", "prüfen"],
  };
...
function buildKnowledgeCore(data) {
  ...
  if (/wärme|ausdehn/i.test(data.thema)) {
    return {
      ...
      workedExample: { situation: "Ein festsitzender Metalldeckel wird unter warmes Wasser gehalten.", observation: "Der Deckel lässt sich danach leichter öffnen.", explanation: "Das Metall erwärmt sich und dehnt sich etwas aus. Dadurch sitzt der Deckel lockerer." },
  ...
  if (/stoff/i.test(data.thema)) {
    return {
      ...
      workedExample: { situation: "Eine unbekannte Probe ist weiß und körnig.", observation: "Sie löst sich in Wasser und wird nicht vom Magneten angezogen.", explanation: "Die Eigenschaften passen eher zu Zucker oder Salz als zu Eisen." },
  ...
  return {
    ...
    terms: suppliedTerms.length ? suppliedTerms.slice(0, 4).map((term) => ({ term, explanation: "Erkläre den Begriff in eigenen Worten.", example: "Finde ein passendes Beispiel." })) : [
    ...
    workedExample: { situation: `Ein Beispiel zu ${data.thema}`, observation: "Beobachtung oder Information notieren.", explanation: "Mit Fachbegriffen erklären und begründen." },
    everydayConnections: ["Alltagsbeispiel finden", "Fachbegriff anwenden", "Ergebnis begründen"],
  };
}

function buildPages(data, pkg, count) {
  const pages = [buildOrientationPage(pkg), buildKnowledgePage(pkg), buildPracticePage(pkg), buildSecuringPage(pkg)];
  if (count === 2) return [pages[0], renumberPage(pages[3], 2)];
  if (count === 6) return [pages[0], pages[1], buildGuidedPracticePage(pkg), buildPracticePage(pkg, 4), buildDifferentiationPage(pkg, 5), buildSecuringPage(pkg, 6)];
  return pages;
}
function renumberPage(pageObject, pageNumber) { return { ...pageObject, pageNumber }; }
...
function buildGuidedPracticePage(pkg) {
  return page(3, PAGE_PURPOSES.PRACTICE, "Gemeinsam nach Muster arbeiten", [
    createBlock(BLOCK_TYPES.SHORT_ANSWER_TASK, { title: "Beispiel übertragen", prompt: "Nutze das ausgefüllte Beispiel. Übertrage das Muster auf ein neues Beispiel.", answerLines: 4, sentenceStarter: "Das passt, weil …" }),
...
function buildPracticePage(pkg, pageNumber = 3) {
  ...
  if (mode === TEMPLATE_MODES.EXPLORATION) return page(pageNumber, PAGE_PURPOSES.PRACTICE, "Recherchieren und bewerten", [
    createBlock(BLOCK_TYPES.RESEARCH_TASK, { title: "Rechercheauftrag", prompt: `Recherchiere zur Stundenfrage: ${pkg.didacticPlan.guidingQuestion}`, criteria: ["passt zur Stundenfrage", "Quelle/Fundort ist notiert", "Aussage ist verständlich"], rows: 4 }),
    createBlock(BLOCK_TYPES.MATCHING_TABLE_TASK, { title: "Ergebnisse vergleichen", columns: ["Kriterium", "Ergebnis A", "Ergebnis B", "Bewertung"], rows: ["passt zur Frage", "verständlich", "wichtig"] }),
  ]);
  if (mode === TEMPLATE_MODES.EXPERIMENT) return page(pageNumber, PAGE_PURPOSES.PRACTICE, "Untersuchen und auswerten", [
    createBlock(BLOCK_TYPES.EXPERIMENT_PROTOCOL, { title: "Versuchsprotokoll", question: pkg.didacticPlan.guidingQuestion, fields: ["Vermutung", "Material", "Durchführung", "Beobachtung", "Auswertung"] }),
    createBlock(BLOCK_TYPES.OBSERVATION_TABLE, { title: "Beobachtungstabelle", columns: ["Schritt", "Beobachtung", "Erklärung"], rows: 4 }),
  ]);
  return page(pageNumber, PAGE_PURPOSES.PRACTICE, "Verstehen und anwenden", [
    createBlock(BLOCK_TYPES.PREDICTION_TASK, { title: "Vorhersage", prompt: `Was erwartest du bei ${pkg.meta.title}? Begründe deine Vermutung.`, options: ["Ich stimme zu", "Ich bin unsicher", "Ich stimme nicht zu"], sentenceStarter: "Ich vermute, dass …, weil …" }),
    createBlock(BLOCK_TYPES.TRUE_FALSE_CORRECTION, { title: "Richtig oder falsch?", items: buildTrueFalseItems(pkg) }),
    createBlock(BLOCK_TYPES.MATCHING_TABLE_TASK, { title: "Anwenden und begründen", columns: ["Situation", "Fachwort", "Begründung"], rows: buildMatchingRows(pkg) }),
  ]);
}

function buildDifferentiationPage(pkg, pageNumber) {
  return page(pageNumber, PAGE_PURPOSES.DIFFERENTIATION, "Wähle dein Niveau", [
    createBlock(BLOCK_TYPES.SHORT_ANSWER_TASK, { title: "★ Stern", prompt: "Bearbeite die Pflichtaufgabe mit Satzstarter.", answerLines: 4, sentenceStarter: "Ich erkenne …, weil …" }),
    createBlock(BLOCK_TYPES.SHORT_ANSWER_TASK, { title: "● Planet", prompt: "Bearbeite eine weitere Situation und begründe selbstständig.", answerLines: 5, sentenceStarter: "Meine Begründung lautet …" }),
    createBlock(BLOCK_TYPES.SHORT_ANSWER_TASK, { title: "▲ Rakete", prompt: "Erfinde ein eigenes Beispiel oder eine kritische Frage.", answerLines: 5, sentenceStarter: "Meine Zusatzidee ist …" }),
  ]);
}

function buildSecuringPage(pkg, pageNumber = 4) {
  return page(pageNumber, PAGE_PURPOSES.SECURING, "Sichern und abgeben", [
    createBlock(BLOCK_TYPES.MERKSATZ_TASK, { title: "Merksatz", prompt: "Schreibe einen fachlich richtigen Merksatz in ein bis zwei Sätzen.", sentenceStarter: "Ich merke mir: …" }),
    createBlock(BLOCK_TYPES.REFLECTION_TASK, { title: "Reflexion", prompts: ["Das habe ich verstanden:", "Das ist noch unsicher:", "Ein Beispiel aus dem Alltag:"] }),
...
function learningStepsFor(mode) {
  if (mode === TEMPLATE_MODES.EXPLORATION) return ["Stundenfrage lesen", "Suchfrage formulieren", "Quelle/Fundort notieren", "Ergebnisse vergleichen", "Bewertung schreiben"];
  ...
  return ["Stundenfrage lesen", "Wissenskasten lesen", "Beispiel nachvollziehen", "Aufgaben bearbeiten", "Merksatz schreiben"];
}

function buildTrueFalseItems(pkg) {
  if (/wärme|ausdehn/i.test(pkg.meta.title)) return [
...
function buildTeacherNotes(data, pkg) { return { flow: pkg.didacticPlan.learningPath, pitfalls: pkg.didacticPlan.misconceptions, differentiation: data.hinweise || "Stern-Aufgabe für schwächere Lernende, Rakete für schnelle Lernende nutzen." }; }
function buildScobees(data, pkg) { return { title: pkg.meta.title, instructions: `Bearbeite das Material zu ${pkg.meta.title} der Reihe nach.`, submission: data.scobeesSubmission || "Lade die Sicherungsseite oder das vollständige Material gut lesbar in Scobees hoch." }; }
